            ...
            <div className="bg-white rounded-[24px] border border-black/5 shadow-sm overflow-hidden">
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-brand-bg/50 border-b border-black/5">
                                <th className="px-6 py-4 text-[0.7rem] font-800 text-gray-400 uppercase tracking-widest">Asset</th>
                                <th className="px-6 py-4 text-[0.7rem] font-800 text-gray-400 uppercase tracking-widest">Specifications</th>
                                <th className="px-6 py-4 text-[0.7rem] font-800 text-gray-400 uppercase tracking-widest">Category</th>
                                <th className="px-6 py-4 text-[0.7rem] font-800 text-gray-400 uppercase tracking-widest">Price</th>
                                <th className="px-6 py-4 text-[0.7rem] font-800 text-gray-400 uppercase tracking-widest">Stock Level</th>
                                <th className="px-6 py-4 text-[0.7rem] font-800 text-gray-400 uppercase tracking-widest text-right">Actions</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-black/5">
                            {filteredProducts.map(p => (
                                <tr key={p.id} className="hover:bg-brand-bg/30 transition-colors group">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className="inline-block bg-brand-bg px-2.5 py-1 rounded-md text-[0.75rem] font-700 text-secondary border border-black/5">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className={`text-xs font-800 ${p.stock < 10 ? 'text-brand-red' : 'text-gray-400'}`}>{p.stock}</span>
                                        </div>
                                    </td>
                ...
                {filteredProducts.length === 0 && (
                    <div className="p-20 text-center">
                        ...
                        <p className="text-secondary font-600">No products match your current search parameters.</p>
                    </div>
                )}
            </div>
                    ...
                    <div className="relative bg-white w-full max-w-2xl rounded-[32px] overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300">
                        ...
                        <div className="p-8 grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div className="sm:col-span-2">
                                <label className="block text-xs font-800 text-gray-400 uppercase tracking-widest mb-2">Instrument Name</label>
                                <input
                                    type="text"
                                    className="w-full bg-brand-bg border border-black/5 rounded-xl px-4 py-3 outline-none focus:border-brand-green font-600 text-brand-text"
                                    value={form.name}
                                    onChange={e => setForm({ ...form, name: e.target.value })}
                                />
                            </div>
                            <div className="sm:col-span-2">
                                <label className="block text-xs font-800 text-gray-400 uppercase tracking-widest mb-2">Technical Description</label>
                                <textarea
                                    className="w-full bg-brand-bg border border-black/5 rounded-xl px-4 py-3 outline-none focus:border-brand-green font-600 text-brand-text min-h-[100px] resize-none"
                                    value={form.description}
                                    onChange={e => setForm({ ...form, description: e.target.value })}
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-800 text-gray-400 uppercase tracking-widest mb-2">Unit Price ($)</label>
                                <input
                                    type="number"
                                    className="w-full bg-brand-bg border border-black/5 rounded-xl px-4 py-3 outline-none focus:border-brand-green font-800 text-brand-green"
                                    value={form.price}
                                    onChange={e => setForm({ ...form, price: e.target.value })}
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-800 text-gray-400 uppercase tracking-widest mb-2">Stock Inventory</label>
                                <input
                                    type="number"
                                    className="w-full bg-brand-bg border border-black/5 rounded-xl px-4 py-3 outline-none focus:border-brand-green font-600 text-brand-text"
                                    value={form.stock}
                            ...
                            <div className="sm:col-span-2">
                                <label className="block text-xs font-800 text-gray-400 uppercase tracking-widest mb-2">Classification / Category</label>
                                <select
                                    className="w-full bg-brand-bg border border-black/5 rounded-xl px-4 py-3 outline-none focus:border-brand-green font-600 text-brand-text appearance-none"
                            ...
                            <div className="sm:col-span-2">
                                <label className="block text-xs font-800 text-gray-400 uppercase tracking-widest mb-2">Media Asset URL</label>
